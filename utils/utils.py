...
def copy_occs(root):    
    ...
    def remember_name(component):
        for saved_component, _ in renamed_components:
            if saved_component == component:
                return
        renamed_components.append((component, component.name))
    ...
    def copy_body(allOccs, occs):
        ...
        copied_occs.append(new_occs)
        if occs.component.name == 'base_link':
            remember_name(occs.component)
    ...
    return copied_occs, renamed_components


def cleanup_copied_occs(copied_occs, renamed_components):
    """
    Restore the Fusion design after temporary STL export components were made.
    """
    for occs in copied_occs:
        try:
            occs.component.name = 'exported_' + occs.component.name
        except:
            pass

    for component, original_name in renamed_components:
        try:
