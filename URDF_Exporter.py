...
def run(context):
    ui = None
    success_msg = 'Successfully created ROS 2 Jazzy URDF package'
    msg = success_msg
    ...
    try:
        ...
        # --------------------
        # Generate URDF, Xacro and ROS 2 package files
        ...
        Write.write_gazebo_launch(package_name, robot_name, save_dir, joints_dict)
        ...
        renamed_components = []
        try:
            copied_occs, renamed_components = utils.copy_occs(root)
            utils.export_stl(design, save_dir, components)
        finally:
            utils.cleanup_copied_occs(copied_occs, renamed_components)
